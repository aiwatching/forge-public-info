#!/usr/bin/env bash
#
# Forge dependencies ONLY — macOS + Linux.
#
# Installs just the prerequisites Forge needs (NOT Forge itself):
#   Required: node ≥ 22.13, pnpm, tmux, git, an agent CLI (claude/codex/aider)
#   Optional: jq, glab, gh   (used by some pipelines)
# Does NOT install @aion0/forge or start the server — run install-forge-local.sh
# (or `npm install -g @aion0/forge`) afterwards.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/aiwatching/forge-public-info/main/install-deps.sh | bash
#   # or
#   bash install-deps.sh                 # interactive (prompts before brew/apt)
#   bash install-deps.sh --yes           # non-interactive
#   bash install-deps.sh --skip-optional
#
...
c_green " Forge dependencies ready (Forge itself NOT installed)."

c_green "──────────────────────────────────────────────────"

echo ""

echo "Next: install + start Forge with either —"

echo "  curl -fsSL https://raw.githubusercontent.com/aiwatching/forge-public-info/main/install-forge-local.sh | bash"

echo "  # or just:  npm install -g @aion0/forge  &&  forge server start"
