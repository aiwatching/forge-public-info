...
# Subshell with pipefail off so the harmless SIGPIPE from `tr` after `head`
# has read enough doesn't tank `set -e`. Without this wrapper the whole
# install aborts silently right here, the container never gets created,
# and the operator sees "Pull complete" followed by nothing.
gen_pw() { ( set +o pipefail; LC_ALL=C tr -dc 'A-Za-z0-9' </dev/urandom | head -c "$1" ); }
[ -z "$ADMIN_PASSWORD" ] && ADMIN_PASSWORD=$(gen_pw 16)
[ -z "$FORGE_PASSWORD" ] && FORGE_PASSWORD=$(gen_pw 12)
...
# --- Persist credentials to a file so the operator can always find them ---
# Goes in the data dir (which is bind-mounted from the host), readable only
# by them. Re-runs of install-portal.sh overwrite this file with whatever
# the current run produced - that way `--admin-password` / `--portal-password`
# overrides land here too.
CREDS_FILE="$WORKSPACE_DIR/.credentials"
umask 077
cat > "$CREDS_FILE" <<EOF
# Forge Portal credentials. Generated by install-portal.sh.
# Re-run install-portal.sh to regenerate / overwrite. Treat as a password file.

# Open this URL in a browser. Portal mode redirects / -> /portal
# automatically, so http://localhost:$HOST_PORT also works.
PORTAL_URL=http://localhost:$HOST_PORT/portal
PORTAL_LOGIN_USERNAME=$SLUG
PORTAL_LOGIN_PASSWORD=$FORGE_PASSWORD

# Admin console for management (add/remove workspaces, change limits, ...).
# Most users never need this in portal mode.
ADMIN_URL=http://localhost:$HOST_PORT/admin
ADMIN_EMAIL=$ADMIN_EMAIL
ADMIN_PASSWORD=$ADMIN_PASSWORD

DATA_DIR=$WORKSPACE_DIR
$([ -n "$PROJECT_DIR" ] && printf 'PROJECT_MOUNT=%s\n' "$PROJECT_DIR")
EOF
...
$(c_grn 'OK') Forge Portal is starting.
  ...
  Open:     $(c_grn "http://localhost:$HOST_PORT")
            (redirects to /portal automatically in portal mode)
  Login:    $(c_grn "$SLUG") / $(c_grn "$FORGE_PASSWORD")

  Admin console (rarely needed):
...
Credentials are also saved to:
  $CREDS_FILE
(cat that file any time to recover. Mode 600 - only you can read.)
...
Day-to-day:
  ...
  cat $CREDS_FILE               # show login info again
EOF
